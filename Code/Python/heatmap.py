...
df = pd.read_csv(
  data_path + "hdtf_binary_gene_avg.csv"
)


# Removes glia from data
...
# Get clusters with same TF expression and creates a list of lists
...
# Combines lists into single list
cluster_combos = list(itertools.chain(*clusters_list_of_lists))

# Adds clusters with unique TF expression to end of list
for i in neuron_clusters:
  if i not in cluster_combos:
    cluster_combos.append(i)


# Converts custom order to str with leading zeros
...
# Function that allows us to sort later
def indexer(e): 
  print(e)
...
# Sums each gene's binary score for each cluster
sums = np.array(df.iloc[:,1:].sum(axis=1))


# Inserts a temporary sum column for us to sort rows
df.insert(
  loc=0,
  column="temp_sum",
  value=sums
...
# Creates index from sorted values
df = df.sort_index(
  axis=1, 
